...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for word in dict.fromkeys(chain.from_iterable(sentence.split() for sentence in sentences)):
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    get=token_to_id.get #Avoid attribute lookup multiple times
    return [get(word,unk_id) for word in sentence.split()]
...
def build_position_index_column(max_len):
    ...
    col=torch.arange(0,max_len,dtype=torch.float)
    return torch.reshape(col,(max_len,1))
...
def build_padding_mask(token_ids, pad_id):
    ...
    B,L=token_ids.shape
    bool_mask=token_ids!=pad_id
    return bool_mask.reshape(B,1,1,L)
...
def softmax_attention_weights(masked_scores):
    ...
    weights= torch.softmax(masked_scores,axis=-1)
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    atten_weights=softmax_attention_weights(scores)
    context=apply_attention_weights_to_values(atten_weights,value)
...
def transpose_heads_before_sequence(split_tensor):
    ...
    return torch.transpose(split_tensor,2,1)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    return multi_head_tensor.transpose(1,2).reshape(B,L,num_heads*d_k)
...
def merge_heads_and_project_output(context, w_o, b_o):
    scores=merge_heads_back_to_model_dim(context)
    return apply_linear_projection(scores,w_o,b_o)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    ...
    q=transpose_heads_before_sequence(
        split_last_dim_into_heads(q,num_heads)
    )

    k=transpose_heads_before_sequence(
        split_last_dim_into_heads(k,num_heads)
    )

    v=transpose_heads_before_sequence(
...
def apply_ffn_first_linear_and_relu(x, w1, b1):
    ...
    linear= apply_linear_projection(x,w1.T,b1)
...
def apply_ffn_second_linear(hidden, w2, b2):
    ...
    return apply_linear_projection(hidden,w2.T,b2)
...
def position_wise_feed_forward_network(x, w1, b1, w2, b2):
    ...
    hidden1=apply_ffn_first_linear_and_relu(x,w1,b1)
    return apply_ffn_second_linear(hidden1,w2,b2)
...
def compute_layer_norm_mean_and_variance(x):
    ...
    variance=torch.var(x,axis=-1,keepdim=True,unbiased=False)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    return apply_residual_add_and_norm(x,mha,gamma,beta)
...
def encoder_layer_feed_forward_sublayer(x, w1, b1, w2, b2, gamma, beta):
    ...
    h1=position_wise_feed_forward_network(x,w1,b1,w2,b2)
    return apply_residual_add_and_norm(x,h1,gamma,beta)
...
def assemble_encoder_layer(x, layer_params, num_heads, src_mask):
    ...
    attn_block=encoder_layer_self_attention_sublayer(x,layer_params['w_q'],layer_params['w_k'],layer_params['w_v'],layer_params['w_o'],layer_params['attn_gamma'],layer_params['attn_beta'],num_heads,src_mask)
    out=encoder_layer_feed_forward_sublayer(attn_block,layer_params['w1'],layer_params['b1'],layer_params['w2'],layer_params['b2'],layer_params['ffn_gamma'],layer_params['ffn_beta'])
    return out
...
def compute_token_accuracy_ignoring_pad(log_probabilities, gold_token_ids, pad_id):
    ...
    return (
        ((pred==gold_token_ids)&mask).sum().float()/count.float()
        if count>0
...
def apply_adam_bias_correction(m_t, v_t, beta1, beta2, step):
    ...
    m_t=m_t/(1-beta1**step)
    v_t=v_t/(1-beta2**step)
    return m_t,v_t
...
def run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step_number, config):
    ...
    beta1=config.get("adam_beta1",config.get("beta1",0.9))
    beta2=config.get("adam_beta2",config.get("beta2",0.98))
    epsilon=config.get("adam_epsilon",config.get("epsilon",1e-9))
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    parent_seq=beam_sequences[beam_indices]
    return torch.cat([parent_seq,token_ids.unsqueeze(1)], dim=1)
